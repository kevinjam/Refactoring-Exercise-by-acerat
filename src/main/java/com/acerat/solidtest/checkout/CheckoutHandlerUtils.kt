...
class CheckoutHandlerUtils {
    ...
    fun isValidShippingInformation(checkoutState: CheckoutState, customer: Customer): Boolean {
    ...
    fun isCheckoutState(checkoutState: CheckoutState, order: Order, customer: Customer): Boolean {
        ...
        if (!checkoutState.isPaid) {
            ...
            if (customer.configuration!!.paymentMenthod == CustomerPaymentMethod.CARD) {
                ...
                if (CARD_PAYMENT_SERVICE.chargeCreditCard(getCurrentCardDetails(customer).get())!!.succeeded()) {
                    checkoutState.cardPaymentCompletedUsing(getCurrentCardDetails(customer).get().cardDetailsReference)
                } else {
                    checkoutState.cardPaymentFailed(CardPaymentFailures.COULD_NOT_COMPLETE_CARD_PAYMENT)
                    return false
                }
            } else if (customer.configuration!!.paymentMenthod == CustomerPaymentMethod.INVOICE) {
                ...
                checkoutState.invoiceSentSuccessfully(INVOICE_HANDLER.produceInvoice(order, customer)!!.invoiceId)
            }
        }
        return true
    }
